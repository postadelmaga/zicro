//! # zicro.wl — hand-written Wayland client bindings
//!
//! The thin FFI floor of zicro, in the same spirit as zicro's `gpu_wgpu_bridge_ffi`:
...
/// The C `wl_array` (e.g. the state list in `xdg_toplevel.configure`): a length-prefixed
/// blob whose `data` points at `size` bytes. Layout must match `<wayland-util.h>`.
...
pub const XdgToplevel = opaque {
    ...
    /// set_fullscreen (opcode 11): null output = the compositor picks the output.
