...
pub const ModuleCtx = struct {
    ...
    pub fn offload(ctx: *ModuleCtx, comptime function: anytype, args: anytype) Allocator.Error!void {
        ...
        // A rejected job (pool shutting down) is never run, so its closure would leak —
        // reclaim it here rather than relying on Closure.call's deferred destroy.
        if (!try ctx.pool.submit(.{ .call = Closure.call, .ctx = closure })) {
            ctx.gpa.destroy(closure);
        }
    }
};
